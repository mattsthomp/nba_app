...
def loadGame
  ...
  if Game.find_by game_date:game_date, home_team:game_home_team
    ...
    event_stack.each do |e|
      if e.attr("action_type") == 0 and e.attr("msg_type") == 4
      elsif e.attr("action_type") == 1 and e.attr("msg_type") == 2
      elsif e.attr("action_type") == 1 and e.attr("msg_type") == 1
      elsif e.attr("action_type") == 0 and e.attr("msg_type") == 8
      elsif e.attr("action_type") == 1 and e.attr("msg_type") == 6
      else  Event.create( )
      end
    end
#   event_stack.each do |e|
#      Event.create( event_num: e.attr("eventid"), player_code: e.attr("player_code"), team: e.attr("tm"), home_score: e.attr("htms"), visitor_score: e.attr("vtms") )
#    end
